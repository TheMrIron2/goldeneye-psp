...
randomfloat(  float l, float h )
=================
*/
void PF_randomfloat (void)
{
	float		num;
	float       flLow;
	float       flHigh;

	flLow  = G_FLOAT(OFS_PARM0);
	flHigh = G_FLOAT(OFS_PARM1);

	num = RandomFloat( flLow, flHigh );
...
randomlong(int l, int h)
=================
*/
void PF_randomlong (void)
{
	float	  num;
	float     lLow;
	float     lHigh;

	lLow  = G_FLOAT(OFS_PARM0);
	lHigh = G_FLOAT(OFS_PARM1);
	
	num = RandomLong( lLow, lHigh );
...
string( edict_t *ent, const vec3_t start, const vec3_t end )
//Uncle Mike, Crow_bar
===============
*/
void PF_tracetexture(void)
{
    edict_t *ent;
    float *start;
    float *end;
	...
	ret = SV_TraceTexture( ent, start, end );

	sprintf(pr_string_temp,"%s",ret);
	G_INT(OFS_RETURN) = pr_string_temp - pr_strings;
}
...
void PF_tracematerial(void)
{
	...
	char *temp = SV_TraceTexture( ent, start, end );
	if (*temp == '-' || *temp == '+')
		 temp += 2;
	if (*temp == '{' || *temp == '!' || *temp == '~' || *temp == ' ')
		temp++;
	ret = MaterialsCheck(temp);

	sprintf(pr_string_temp,"%c",ret);
	G_INT(OFS_RETURN) = pr_string_temp - pr_strings;
}
...
//	{   5, "fixme", PF_Fixme },				// void(entity e, vector min, vector max) setabssize = #5;
	...
	{  33, "randomlong ", PF_randomlong },				//float randomlong( float l, float h )  
	{  34, "droptofloor", PF_droptofloor },
	{  35, "lightstyle", PF_lightstyle },
	{  36, "rint", PF_rint },
	{  37, "floor", PF_floor },
	{  38, "ceil", PF_ceil },
	{  39, "tracearea", PF_tracearea }, // float(vector v1, vector v2, vector mins, vector maxs,
	{  40, "checkbottom", PF_checkbottom },
	{  41, "pointcontents", PF_pointcontents },
	{  42, "tracetexture", PF_tracetexture },// string( entity e, vector start, vector end )
	{  43, "fabs", PF_fabs },
	{  44, "aim", PF_aim },
	{  45, "cvar", PF_cvar },
	...
	{  50, "randomfloat", PF_randomfloat },//float randomfloat( float l, float h ) 
	...
	{  79, "vlen2d", PF_vlen2d },
	{  80, "tracematerial", PF_tracematerial },
	{  81, "stof", PF_stof },
