...
void SV_Physics_Pusher (edict_t *ent)
{
	...
	if( movetime )
	{
		if( VecLength2( ent->v.avelocity ) > STOP_EPSILON )
		{
			if( VecLength2( ent->v.velocity ) > STOP_EPSILON )
			{
...
void SV_Physics_Compound( edict_t *ent )
{
	...
	parent = PROG_TO_EDICT(ent->v.aiment);

	if(!(parent && !parent->free)) return;

	if( ent->v.solid != SOLID_TRIGGER )
		ent->v.solid = SOLID_NOT;


	if(parent->v.movetype != MOVETYPE_PUSH)
	    return;
	...
	if( ent->v.ltime == 0.0f )
	{
		...
		ent->v.ltime = parent->v.ltime ? parent->v.ltime : host_frametime;
		return;
	}
