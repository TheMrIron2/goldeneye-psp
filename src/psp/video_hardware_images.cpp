...
int loadtextureimage (char* filename, int matchwidth, int matchheight, qboolean complain, int filter)
{
	...
	{
       texture_index = GL_LoadPaletteTexture (filename, image_width, image_height, data, (byte *)image_palette, image_palette_type, qtrue, filter, 0);
	   memset(image_palette, 0, sizeof(image_palette)); // clear temp pal
       image_palette_type = 0;
	}
    else
    {
       texture_index = GL_LoadTexture (filename, image_width, image_height, data, 4, qtrue, filter, 0);
	}
    free(data);     
