...
void Mod_LoadQ2SpriteModel (model_t *mod, void *buffer);
		...
		else
		{
			...
			else
			{
				...
				if (r_mipmaps.value > 0)
					level = 3;
				tx->gl_texturenum = GL_LoadTexture (mt->name, tx->width, tx->height, (byte *)(tx_pixels), 1, qtrue, GU_LINEAR, level);
				mapTextureNameList.push_back(tx->gl_texturenum);
        	}
		}
		free (tx_pixels);
...
void *Mod_LoadAllSkins (int numskins, daliasskintype_t *pskintype)
{
	...
	for (i=0 ; i<numskins ; i++)
	{
		if (pskintype->type == ALIAS_SKIN_SINGLE) {
			...
			int gl_ident = GL_LoadTexture (name, pheader->skinwidth, pheader->skinheight, (byte *)(pskintype + 1), 1, qtrue, GU_LINEAR, 0);
			...
			for (j=0 ; j<groupskins ; j++)
			{
					...
					pheader->gl_texturenum[i][j&3] = 
						...
						pheader->skinheight, (byte *)(pskintype), 1, qtrue, GU_LINEAR, 0);
...
=================
*/
void * Mod_LoadSpriteFrame (void * pin, mspriteframe_t **ppframe, int framenum, int version)
{
...
=================
*/
void * Mod_LoadSpriteGroup (void * pin, mspriteframe_t **ppframe, int framenum, int version)
{
...
*/
      ...
      case SPRITE32_VERSION:
		break;
	  default:
       Sys_Error ("%s has wrong version number (%i should be %i(q1),%i(dp))", mod->name, version, SPRITE_VERSION, HLSPRITE_VERSION, SPRITE32_VERSION);
	}
